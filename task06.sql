-- 1
-- В результате выполнения запроса выведутся участники, у которых суммы транзакций больше 50
...
-- 2
-- В результате выполнения запроса для каждого участника выведется количество сыгранных игр
...
-- 3
-- В результате выполнения запроса выведется среднее значение сумм транзакций для каждого участника
select player_id,
...
-- 4
-- В результате выполнения запроса для каждого стола выведется количество игр, в которых он участвовал
select distinct
    e.table_id,
...
-- 5
-- В результате выполнения запроса для каждого участника выведется игра, в которую тот играл больше всего раз
select
    p.player_id,
    p.nickname,
    (
...
-- 6
-- В результате выполнения запроса выведется для каждой транзакции участника информация о предыдущей транзакции
select
    ct.player_id,
    ct.transaction_id,
    ct.transaction_date,
